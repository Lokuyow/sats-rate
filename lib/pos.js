...
export class Pos {
  ...
  async pay() {
   ...
   // TODO: 入力値を固定してLNURLを作成する
   const lightningAddress = new LightningAddress(this._lnAddress);
   await lightningAddress.fetchAddressData();

   if(lightningAddress.data.status === 'OK' && lightningAddress.data.payRequest !== 'payRequest') {
    console.warn(`Pos: invalid lightningAddress`);
    return;
   }
  }
  ...
  #showQrCode() {
    ...
    img.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=lightning://${this._lnAddress}`
